...
export class MenuPostagem {
  ...
  nomeMes = computed(()=>this.mesExibido().toLocaleDateString('pt-BR',{month:'long',year:'numeric'}));

  dataSelecionadaFormatada = computed(()=>this.dataSelecionada().toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'}));
  ...
  eventosDoDia = computed(()=>this.eventos().filter(evento=>this.mesmaData(evento.data,this.dataSelecionada())));
